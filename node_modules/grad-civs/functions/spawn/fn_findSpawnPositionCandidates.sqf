...
private _halfSectorWidth = _maxSpawnDistance - _minSpawnDistance;
...
_refPositions = _searchDirs apply {
    ...
    [
        (_refPos select 0) + (_minDistance + _maxDistance / 2) * sin _x,
        (_refPos select 1) + (_minDistance + _maxDistance / 2) * cos _vx
    ];
};
...
private _candidates = switch (_mode) do {
    case "road": {
        ...
        _refPositions apply {
            _x params ["_a", "_b"];
            _roadSegments = [_a, _b] nearRoads _halfSectorWidth;
