...
for [{_i=0}, {_i<1}, {_i=_i+1}] do {
    ...
    _inExclusionZone = {
        if (_roadPos inArea _x) exitWith {true};
        false
    } forEach GRAD_CIVS_EXCLUSION_ZONES;

    _awayFromOtherCivs = {
        if (_roadPos distance _x < 100) exitWith {false};
        true
    } forEach _civilians + [[0, 0, 0]];

    diag_log format["road %1, spawn %2, away %3, excl %4", _roadSegment,  _inSpawnZone, _awayFromOtherCivs,  _inExclusionZone];

    if (!isNull _roadSegment && _inSpawnZone && _awayFromOtherCivs && !_inExclusionZone) exitWith {
        _return = _roadPos;
    };
};
